  ...
  return (
    <div className="absolute inset-0 flex flex-col pt-[52px] text-foreground overflow-y-auto scrollbar-none"
      ...
      <div className="relative z-10 px-6 flex items-center justify-between">
        <span className="neko-text-card-title text-[oklch(0.55_0.04_300)]">‹</span>
        <span className="neko-text-tiny text-[oklch(0.55_0.04_300)]">喵一下人格档案</span>
        <span className="neko-text-support">⋯</span>
      </div>
      ...
      <div className="relative z-10 mx-auto mt-5 h-[152px] w-[152px]">
        ...
        <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 rounded-full bg-white px-3 py-1 neko-text-badge text-[oklch(0.55_0.1_320)] shadow-[0_4px_12px_-4px_oklch(0.78_0.11_305/0.4)]">
          ✦ SYNC 87.4%
        </span>
      </div>

      <div className="relative z-10 mt-8 text-center">
        <div className="neko-text-badge text-[oklch(0.6_0.08_320)]">高冷观察者 · INTJ-A</div>
        <h2 className="mt-2 neko-text-page-title">糯 米 团</h2>
        <div className="mt-1 neko-text-tiny text-[oklch(0.58_0.04_300)]">3 岁 · 金吉拉 · ♀</div>
      </div>

      {/* keywords */}
      <div className="relative z-10 mt-4 flex flex-wrap justify-center gap-1.5 px-6">
        {["安静", "观察者", "夜行性", "傲娇", "撒娇高手", "粘人小怪"].map((k, i) => (
          <span key={k} className="rounded-full px-3 py-1 neko-text-badge"
            style={{
      ...
      <div className="relative z-10 mt-5 mx-5 rounded-[28px] p-5 glass">
        <div className="flex items-center justify-between mb-3">
          <span className="neko-text-tiny text-[oklch(0.55_0.06_300)]">人格指数</span>
          <span className="neko-text-tiny text-[oklch(0.6_0.1_320)]">本周更新</span>
        </div>
        <div className="space-y-3">
          {stats.map((s) => (
            <div key={s.label}>
              <div className="flex items-center justify-between neko-text-tiny">
                <span className="text-foreground/90">{s.label}</span>
                <span className="text-[oklch(0.55_0.05_300)]">{s.value}</span>
              </div>
      ...
      <div className="relative z-10 mx-5 mt-4 rounded-[28px] p-5"
        ...
        <div className="neko-text-tiny text-[oklch(0.55_0.08_320)]">AI 内心独白</div>
        <p className="mt-3 neko-text-support text-foreground">
          “我喜欢你打字的声音，<br />那意味着你还在房间里。”
        </p>
        <div className="mt-3 flex items-center justify-between neko-text-tiny text-[oklch(0.55_0.06_300)]">
          <span>— 糯米团 · 02:11</span>
  ...
  return (
    <button className="relative overflow-hidden rounded-[22px] bg-white/85 p-4 text-left shadow-[0_8px_20px_-12px_oklch(0.78_0.11_305/0.3)]">
      <div className="flex items-center justify-between">
        <div className="neko-text-tiny text-[oklch(0.6_0.1_320)]">{hint}</div>
        <span className="neko-text-body">{emoji}</span>
      </div>
      <div className="mt-2 neko-text-support text-foreground">{title}</div>
