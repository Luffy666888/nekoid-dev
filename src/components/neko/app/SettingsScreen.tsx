    ...
    <ScreenShell>
      ...
      <div className="absolute inset-0 overflow-y-auto px-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-[52px]">
        <div className="flex items-center justify-between">
          ...
          <div className="neko-text-caption text-foreground">设置</div>
        ...
        <div className="mt-6 overflow-hidden rounded-[22px] bg-white/78 backdrop-blur" style={{ border: "1px solid oklch(1 0 0 / 0.75)" }}>
          {rows.map((row, index) => (
            <div key={row.title} className={`flex min-h-[68px] items-center gap-3 px-4 py-3 ${index ? "border-t border-[oklch(0.9_0.02_300)]" : ""}`}>
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl neko-text-support text-[oklch(0.5_0.1_320)]" style={{ background: "linear-gradient(135deg, oklch(0.96 0.04 320), oklch(0.95 0.04 280))" }}>{row.icon}</div>
              <div className="min-w-0 flex-1">
                <div className="neko-text-caption text-foreground">{row.title}</div>
                <div className="mt-0.5 neko-text-tiny text-[oklch(0.55_0.05_300)]">{row.subtitle}</div>
              </div>
        ...
        <div className="mt-6 text-center neko-text-tiny text-[oklch(0.58_0.05_300)]">
